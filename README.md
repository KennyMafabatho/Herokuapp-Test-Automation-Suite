...
Automated UI and API test scripts for the login flow on the-internet.herokuapp.com and the reqres.in mock REST API.
...
This installs selenium-webdriver (used by the UI tests). The API tests use Node's built-in fetch and assert modules — no extra install needed.

Running the UI Tests (Selenium)

UI tests automate a real Chrome browser against the login page, assert on the result, and save a screenshot as evidence.
...
login.spec.js	Successful login	tomsmith / SuperSecretPassword!	Flash message confirms login; screenshot saved as login-success.png
...
Consider adding a .gitignore for generated screenshots (*.png) if committing this repo to version control.
Consider initializing Git (git init) and committing regularly to protect against accidental file/folder deletion.
